...
img = cv2.imread(r'D:\Lab\Telecommunication Lab\lab3\figure4.jpg',0) 
fft = np.fft.fft2(img)
fft_vis = np.abs(fft)
...
fig.add_trace(go.Heatmap(z=fftshift_vis,colorscale='gray',showscale=False),row=1,col=3)
...
fig1.add_trace(go.Surface(x=x,y=y,z=fftshift_vis,colorscale='gray',showscale=False),row=1,col=3)
fig1.show()